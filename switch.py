...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    if "regulation_ph" in CONFIG_ENTITIES:
        switches.append(PoolTechnologieSwitch(coordinator, entry, "regulation_ph", CONFIG_ENTITIES["regulation_ph"]))
...
class PoolTechnologieSwitch(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self):
        ...
        self.coordinator.modbus_client.write_register(self.config_config["address"], 1)
        await self.coordinator.async_request_refresh()

    async def async_turn_off(self):
        """Désactive l'interrupteur."""
        self.coordinator.modbus_client.write_register(self.config_config["address"], 0)
        await self.coordinator.async_request_refresh()
