...
# CORRECTIF : FlowResult est déprécié depuis HA 2024.x → ConfigFlowResult
# (FlowResult reste importable mais lève un DeprecationWarning dans les logs)
try:
    from homeassistant.config_entries import ConfigFlowResult
except ImportError:
    # Compatibilité avec les versions HA antérieures à 2024.4
...
class PoolTechnologieConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    @staticmethod
    @callback
    def async_get_options_flow(config_entry: config_entries.ConfigEntry) -> "PoolTechnologieOptionsFlow":
...
class PoolTechnologieOptionsFlow(config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry: config_entries.ConfigEntry) -> None:
        ...
        # CORRECTIF : dans les versions récentes de HA, config_entry est injecté
        # automatiquement via self.config_entry par la classe parente — on garde
        # l'assignation manuelle pour assurer la compatibilité avec les versions antérieures
    ...
    async def async_step_init(self, user_input=None) -> ConfigFlowResult:
        ...
        options_schema = vol.Schema({
            vol.Required(
                CONF_REGULATION_ORP,
                default=self.config_entry.options.get(CONF_REGULATION_ORP, False),
            ): bool,
        })
